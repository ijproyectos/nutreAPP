...
- `src/app/app/pacientes/[id]/` — **ficha de paciente nueva** (no existía antes de esto — RF-022 solo estaba especificada, no construida). Tiene las secciones Laboratorios (revisar/editar valores/validar/rechazar) y Plan alimentario (ver abajo). El resto de la ficha (historial de turnos, mediciones) queda para cuando se necesite.
...
**Email transaccional con Resend** (RF-020 invitación, RF-042 recordatorio manual de turno — `docs/architecture.md` §, sección "Email transaccional"):
- `src/lib/email/enviar.ts` — único punto de contacto con `resend` (`RESEND_API_KEY` en `.env.local`, `import "server-only"`). Mismo criterio "nunca lanza" que `src/lib/laboratorios/parsear.ts`: si el envío falla, devuelve `{ enviado: false, error }` en vez de tirar — un mail que no sale no puede tumbar el alta de un paciente ni el flujo de recordatorio.
- `crearPaciente` (`pacientes/actions.ts`) manda la invitación por mail además de mostrar el link para copiar — el link sigue siendo el camino principal (RF-020), el mail es un canal adicional no bloqueante. El origin para armar el link se arma server-side con `headers()` (`x-forwarded-host`/`x-forwarded-proto`), no con una env var nueva.
...
- **Sin dominio verificado en Resend** el remitente de sandbox (`onboarding@resend.dev`, o el que esté en `RESEND_FROM_EMAIL`) solo entrega al email dueño de la cuenta de Resend — a un paciente real le va a devolver error (queda logueado, no rompe la UI). Verificar un dominio propio en Resend es lo que falta para que esto funcione con destinatarios reales.
