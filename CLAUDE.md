...
- `supabase/migrations/007_historia_clinica.sql` — agrega `pacientes.notas_generales` (única columna nueva; `mediciones` y `turnos` ya tenían tabla + RLS desde 001/002, solo faltaba UI). **Confirmado NO aplicada contra la DB en vivo** (introspección de solo lectura del schema REST de Supabase con la service role key — `pacientes` en producción no tiene la columna) y esto rompió `/app/pacientes/[id]` con 404 en producción, porque `page.tsx` seleccionaba `notas_generales` y el código no distinguía "error de query" de "paciente no existe". Los dos bugs corregidos:
  - `page.tsx` ahora lee `error` del `select` de `pacientes` y tira un error explícito (logueado) en vez de `notFound()` silencioso cuando la query falla — un 404 legítimo (paciente que no existe) sigue siendo 404, pero un error real de Postgres ya no se disfraza de eso. Mismo criterio aplicado en `generarPlanIA` (`planes-actions.ts`), que tenía el mismo antipatrón.
  - Sigue pendiente aplicar la migración — sin eso, `/app/pacientes/[id]` va a tirar el error explícito (ya no un 404 mudo) y `guardarNotas` va a seguir fallando. Avisar antes de asumir que está aplicada.
